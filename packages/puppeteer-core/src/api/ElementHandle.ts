...
> extends JSHandle<ElementType> {
  ...
  @throwIfDisposed()
  @bindIsolatedHandle
  async touchStart(this: ElementHandle<Element>): Promise<Touch> {
    await this.scrollIntoViewIfNeeded();
    const {x, y} = await this.clickablePoint();
   ...
   * @param touch - An optional Touch. If provided, this touch
   * will be moved. If not provided, the first active touch will be moved.
   */
  @throwIfDisposed()
  @bindIsolatedHandle
  async touchMove(this: ElementHandle<Element>, touch?: Touch): Promise<void> {
    await this.scrollIntoViewIfNeeded();
    const {x, y} = await this.clickablePoint();
    if (touch) {
