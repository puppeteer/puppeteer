...
beforeAll(async state  => {
  ...
  const httpsPort = 8907 + state.parallel * 2 + 1;
  ...
  state.urls = {
    ...
    HTTPS_PREFIX: `https://localhost:${httpsPort}`,
...
describe('Puppeteer', function() {
  describe('Puppeteer.launch', function() {
    it('should support ignoreHTTPSErrors option', async({urls}) => {
      const options = Object.assign({ignoreHTTPSErrors: true}, defaultBrowserOptions);
      const browser = await puppeteer.launch(options);
      const page = await browser.newPage();
      let error = null;
      const response = await page.goto(urls.HTTPS_PREFIX + '/empty.html').catch(e => error = e);
      expect(error).toBe(null);
      expect(response.ok).toBe(true);
      browser.close();
    });
    it('should reject all promises when browser is closed', async({urls}) => {
    ...
    it('should reject if executable path is invalid', async({urls}) => {
      let waitError = null;
    ...
    it('userDataDir option', async({urls}) => {
    ...
    it('userDataDir argument', async({urls}) => {
      const userDataDir = fs.mkdtempSync(path.join(__dirname, 'test-user-data-dir'));
    ...
    it('userDataDir option should restore state', async({urls}) => {
      const userDataDir = fs.mkdtempSync(path.join(__dirname, 'test-user-data-dir'));
      const options = Object.assign({userDataDir}, defaultBrowserOptions);
      const browser = await puppeteer.launch(options);
      const page = await browser.newPage();
      await page.goto(urls.EMPTY_PAGE);
      await page.evaluate(() => localStorage.hey = 'hello');
      await browser.close();

      const browser2 = await puppeteer.launch(options);
      const page2 = await browser2.newPage();
      await page2.goto(urls.EMPTY_PAGE);
      expect(await page2.evaluate(() => localStorage.hey)).toBe('hello');
      await browser2.close();
      rm(userDataDir);
    });
    // @see https://github.com/GoogleChrome/puppeteer/issues/1537
    xit('userDataDir option should restore cookies', async({urls}) => {
      const userDataDir = fs.mkdtempSync(path.join(__dirname, 'test-user-data-dir'));
      const options = Object.assign({userDataDir}, defaultBrowserOptions);
      const browser = await puppeteer.launch(options);
      const page = await browser.newPage();
    ...
    xit('headless should be able to read cookies written by headful', async({urls}) => {
      const userDataDir = fs.mkdtempSync(path.join(__dirname, 'test-user-data-dir'));
      const options = Object.assign({userDataDir}, defaultBrowserOptions);
      ...
      await headfulPage.goto(urls.EMPTY_PAGE);
      ...
      await headlessPage.goto(urls.EMPTY_PAGE);
  ...
  describe('Puppeteer.connect', function() {
    it('should be able to connect multiple times to the same browser', async({urls}) => {
    ...
    it('should be able to reconnect to a disconnected browser', async({urls}) => {
      const originalBrowser = await puppeteer.launch(defaultBrowserOptions);
      const browserWSEndpoint = originalBrowser.wsEndpoint();
      const page = await originalBrowser.newPage();
      await page.goto(urls.PREFIX + '/frames/nested-frames.html');
      originalBrowser.disconnect();

      const FrameUtils = require('./frame-utils');
      const browser = await puppeteer.connect({browserWSEndpoint});
      const pages = await browser.pages();
      const restoredPage = pages.find(page => page.url() === urls.PREFIX + '/frames/nested-frames.html');
...
describe('Page', function() {
  ...
  describe('Page.evaluate', function() {
    it('should work', async({page, urls}) => {
      const result = await page.evaluate(() => 7 * 3);
      expect(result).toBe(21);
    });
    it('should await promise', async({page, urls}) => {
      const result = await page.evaluate(() => Promise.resolve(8 * 7));
      expect(result).toBe(56);
    });
    it('should work right after framenavigated', async({page, urls}) => {
    ...
    it('should work from-inside an exposed function', async({page, urls}) => {
    ...
    it('should reject promise with exception', async({page, urls}) => {
      let error = null;
      await page.evaluate(() => not.existing.object.property).catch(e => error = e);
      expect(error).toBeTruthy();
      expect(error.message).toContain('not is not defined');
    });
    it('should return complex objects', async({page, urls}) => {
      const object = {foo: 'bar!'};
      const result = await page.evaluate(a => a, object);
      expect(result).not.toBe(object);
      expect(result).toEqual(object);
    });
    it('should return NaN', async({page, urls}) => {
      const result = await page.evaluate(() => NaN);
      expect(Object.is(result, NaN)).toBe(true);
    });
    it('should return -0', async({page, urls}) => {
      const result = await page.evaluate(() => -0);
      expect(Object.is(result, -0)).toBe(true);
    });
    it('should return Infinity', async({page, urls}) => {
    ...
    it('should accept "undefined" as one of multiple parameters', async({page, urls}) => {
      const result = await page.evaluate((a, b) => Object.is(a, undefined) && Object.is(b, 'foo'), undefined, 'foo');
      expect(result).toBe(true);
    });
    it('should fail for window object', async({page, urls}) => {
      const result = await page.evaluate(() => window);
      expect(result).toBe(undefined);
    });
    it('should accept a string', async({page, urls}) => {
      const result = await page.evaluate('1 + 2');
      expect(result).toBe(3);
    });
    it('should accept a string with semi colons', async({page, urls}) => {
      const result = await page.evaluate('1 + 5;');
      expect(result).toBe(6);
    });
    it('should accept a string with comments', async({page, urls}) => {
      const result = await page.evaluate('2 + 5;\n// do some math!');
      expect(result).toBe(7);
    });
    it('should accept element handle as an argument', async({page, urls}) => {
      await page.setContent('<section>42</section>');
      const element = await page.$('section');
      const text = await page.evaluate(e => e.textContent, element);
      expect(text).toBe('42');
    });
    it('should throw if underlying element was disposed', async({page, urls}) => {
    ...
    it('should throw if elementHandles are from other frames', async({page, urls}) => {
      const FrameUtils = require('./frame-utils');
      await FrameUtils.attachFrame(page, 'frame1', urls.EMPTY_PAGE);
    ...
    it('should accept object handle as an argument', async({page, urls}) => {
    ...
    it('should accept object handle to primitive types', async({page, urls}) => {
  ...
  describe('Page.setOfflineMode', function() {
    it('should work', async({page, urls}) => {
      ...
      let error = null;
      await page.goto(urls.EMPTY_PAGE).catch(e => error = e);
      expect(error).toBeTruthy();
      await page.setOfflineMode(false);
      const response = await page.reload();
      expect(response.status).toBe(200);
    });
    it('should emulate navigator.onLine', async({page, urls}) => {
  ...
  describe('ExecutionContext.queryObjects', function() {
    ...
    it('should fail for disposed handles', async({page, urls}) => {
    ...
    it('should fail primitive values as prototypes', async({page, urls}) => {
  ...
  describe('JSHandle.jsonValue', function() {
    ...
    it('should not work with dates', async({page, urls}) => {
      const dateHandle = await page.evaluateHandle(() => new Date('2017-09-26T00:00:00.000Z'));
      const json = await dateHandle.jsonValue();
      expect(json).toEqual({});
    });
    it('should throw for circular objects', async({page, urls}) => {
      const windowHandle = await page.evaluateHandle('window');
      let error = null;
  ...
  describe('JSHandle.getProperties', function() {
    ...
    it('should return even non-own properties', async({page, urls}) => {
      const aHandle = await page.evaluateHandle(() => {
        class A {
          constructor() {
            this.a = '1';
  ...
  describe('JSHandle.asElement', function() {
    ...
    it('should return null for non-elements', async({page, urls}) => {
      const aHandle = await page.evaluateHandle(() => 2);
      const element = aHandle.asElement();
      expect(element).toBeFalsy();
    });
    it('should return ElementHandle for TextNodes', async({page, urls}) => {
  ...
  describe('JSHandle.toString', function() {
    it('should work for primitives', async({page, urls}) => {
      const numberHandle = await page.evaluateHandle(() => 2);
      expect(numberHandle.toString()).toBe('JSHandle:2');
      const stringHandle = await page.evaluateHandle(() => 'a');
      expect(stringHandle.toString()).toBe('JSHandle:a');
    });
    it('should work for complicated objects', async({page, urls}) => {
  ...
  describe('Frame.evaluate', function() {
    const FrameUtils = require('./frame-utils');
    it('should have different execution contexts', async({page, urls}) => {
      await page.goto(urls.EMPTY_PAGE);
      await FrameUtils.attachFrame(page, 'frame1', urls.EMPTY_PAGE);
      expect(page.frames().length).toBe(2);
    ...
    it('should execute after cross-site navigation', async({page, urls}) => {
      await page.goto(urls.EMPTY_PAGE);
      const mainFrame = page.mainFrame();
      expect(await mainFrame.evaluate(() => window.location.href)).toContain('localhost');
      await page.goto(urls.CROSS_PROCESS_PREFIX + '/empty.html');
  ...
  describe('Frame.waitForFunction', function() {
    ...
    it('should poll on interval', async({page, urls}) => {
      let success = false;
    ...
    it('should poll on mutation', async({page, urls}) => {
      let success = false;
    ...
    it('should poll on raf', async({page, urls}) => {
      const watchdog = page.waitForFunction(() => window.__FOO === 'hit', {polling: 'raf'});
      await page.evaluate(() => window.__FOO = 'hit');
      await watchdog;
    });
    it('should throw on bad polling value', async({page, urls}) => {
      let error = null;
      try {
    ...
    it('should throw negative polling interval', async({page, urls}) => {
      let error = null;
      try {
  ...
  describe('Frame.waitForSelector', function() {
    ...
    it('should immediately resolve promise if node exists', async({page, urls}) => {
      await page.goto(urls.EMPTY_PAGE);
    ...
    it('should resolve promise when node is added', async({page, urls}) => {
      await page.goto(urls.EMPTY_PAGE);
      const frame = page.mainFrame();
      let added = false;
    ...
    it('should work when node is added through innerHTML', async({page, urls}) => {
      await page.goto(urls.EMPTY_PAGE);
    ...
    it('Page.waitForSelector is shortcut for main frame', async({page, urls}) => {
      await page.goto(urls.EMPTY_PAGE);
      await FrameUtils.attachFrame(page, 'frame1', urls.EMPTY_PAGE);
    ...
    it('should run in specified frame', async({page, urls}) => {
      await FrameUtils.attachFrame(page, 'frame1', urls.EMPTY_PAGE);
      await FrameUtils.attachFrame(page, 'frame2', urls.EMPTY_PAGE);
    ...
    it('should throw if evaluation failed', async({page, urls}) => {
    ...
    it('should throw when frame is detached', async({page, urls}) => {
      await FrameUtils.attachFrame(page, 'frame1', urls.EMPTY_PAGE);
    ...
    it('should survive cross-process navigation', async({page, urls}) => {
      let boxFound = false;
      ...
      await page.reload();
      expect(boxFound).toBe(false);
      await page.goto(urls.CROSS_PROCESS_PREFIX + '/grid.html');
      await waitForSelector;
      expect(boxFound).toBe(true);
    });
    it('should wait for visible', async({page, urls}) => {
      let divFound = false;
    ...
    it('should wait for visible recursively', async({page, urls}) => {
    ...
    it('hidden should wait for visibility: hidden', async({page, urls}) => {
      let divHidden = false;
    ...
    it('hidden should wait for display: none', async({page, urls}) => {
      let divHidden = false;
      await page.setContent(`<div style='display: block;'></div>`);
      const waitForSelector = page.waitForSelector('div', {hidden: true}).then(() => divHidden = true);
      await page.waitForSelector('div'); // do a round trip
    ...
    it('hidden should wait for removal', async({page, urls}) => {
    ...
    it('should respect timeout', async({page, urls}) => {
      let error = null;
    ...
    it('should respond to node attribute mutation', async({page, urls}) => {
      let divFound = false;
  ...
  describe('Page.waitFor', function() {
    it('should wait for selector', async({page, urls}) => {
      let found = false;
      const waitFor = page.waitFor('div').then(() => found = true);
      await page.goto(urls.EMPTY_PAGE);
      expect(found).toBe(false);
      await page.goto(urls.PREFIX + '/grid.html');
      await waitFor;
      expect(found).toBe(true);
    });
    it('should timeout', async({page, urls}) => {
      const startTime = Date.now();
      const timeout = 42;
      await page.waitFor(timeout);
      expect(Date.now() - startTime).not.toBeLessThan(timeout / 2);
    });
    it('should wait for predicate', async({page, urls}) => {
      const watchdog = page.waitFor(() => window.innerWidth < 100);
      page.setViewport({width: 10, height: 10});
      await watchdog;
    });
    it('should throw when unknown type', async({page, urls}) => {
      let error = null;
      await page.waitFor({foo: 'bar'}).catch(e => error = e);
      expect(error.message).toContain('Unsupported target type');
    });
    it('should wait for predicate with arguments', async({page, urls}) => {
  ...
  describe('Page.Events.Console', function() {
    ...
    it('should work for different console API calls', async({page, urls}) => {
      const messages = [];
    ...
    it('should not fail for window object', async({page, urls}) => {
      let message = null;
  ...
  describe('Page.metrics', function() {
    it('should get metrics from a page', async({page, urls}) => {
      await page.goto('about:blank');
      const metrics = await page.metrics();
      checkMetrics(metrics);
    });
    it('metrics event fired on console.timeStamp', async({page, urls}) => {
  ...
  describe('Page.goto', function() {
    it('should navigate to about:blank', async({page, urls}) => {
      const response = await page.goto('about:blank');
      expect(response).toBe(null);
    });
    it('should navigate to empty page with domcontentloaded', async({page, urls}) => {
      const response = await page.goto(urls.EMPTY_PAGE, {waitUntil: 'domcontentloaded'});
      expect(response.status).toBe(200);
    });
    it('should navigate to empty page with networkidle0', async({page, urls}) => {
      const response = await page.goto(urls.EMPTY_PAGE, {waitUntil: 'networkidle0'});
      expect(response.status).toBe(200);
    });
    it('should navigate to empty page with networkidle2', async({page, urls}) => {
      const response = await page.goto(urls.EMPTY_PAGE, {waitUntil: 'networkidle2'});
      expect(response.status).toBe(200);
    });
    it('should fail when navigating to bad url', async({page, urls}) => {
      let error = null;
      await page.goto('asdfasdf').catch(e => error = e);
      expect(error.message).toContain('Cannot navigate to invalid URL');
    });
    it('should fail when navigating to bad SSL', async({page, urls}) => {
      ...
      let error = null;
      await page.goto(urls.HTTPS_PREFIX + '/empty.html').catch(e => error = e);
      expect(error.message).toContain('net::ERR_CERT_AUTHORITY_INVALID');
    });
    it('should fail when navigating to bad SSL after redirects', async({page, urls, server}) => {
      server.setRedirect('/redirect/1.html', '/redirect/2.html');
      server.setRedirect('/redirect/2.html', '/empty.html');
      let error = null;
      await page.goto(urls.HTTPS_PREFIX + '/redirect/1.html').catch(e => error = e);
      expect(error.message).toContain('net::ERR_CERT_AUTHORITY_INVALID');
    });
    it('should throw if networkidle is passed as an option', async({page, urls}) => {
      let error = null;
      await page.goto(urls.EMPTY_PAGE, {waitUntil: 'networkidle'}).catch(err => error = err);
      expect(error.message).toContain('"networkidle" option is no longer supported');
    });
    it('should fail when main resources failed to load', async({page, urls}) => {
      let error = null;
      await page.goto('http://localhost:44123/non-existing-url').catch(e => error = e);
      expect(error.message).toContain('net::ERR_CONNECTION_REFUSED');
    });
    it('should fail when exceeding maximum navigation timeout', async({page, urls, server}) => {
      // Hang for request to the empty.html
      server.setRoute('/empty.html', (req, res) => { });
      let error = null;
      await page.goto(urls.PREFIX + '/empty.html', {timeout: 1}).catch(e => error = e);
      expect(error.message).toContain('Navigation Timeout Exceeded: 1ms');
    });
    it('should disable timeout when its set to 0', async({page, urls}) => {
      let error = null;
      let loaded = false;
      page.once('load', () => loaded = true);
      await page.goto(urls.PREFIX + '/grid.html', {timeout: 0, waitUntil: ['load']}).catch(e => error = e);
      expect(error).toBe(null);
      expect(loaded).toBe(true);
    });
    it('should work when navigating to valid url', async({page, urls}) => {
      const response = await page.goto(urls.EMPTY_PAGE);
      expect(response.ok).toBe(true);
    });
    it('should work when navigating to data url', async({page, urls}) => {
      const response = await page.goto('data:text/html,hello');
      expect(response.ok).toBe(true);
    });
    it('should work when navigating to 404', async({page, urls}) => {
      const response = await page.goto(urls.PREFIX + '/not-found');
      expect(response.ok).toBe(false);
      expect(response.status).toBe(404);
    });
    it('should return last response in redirect chain', async({page, urls, server}) => {
      server.setRedirect('/redirect/1.html', '/redirect/2.html');
      server.setRedirect('/redirect/2.html', '/redirect/3.html');
      server.setRedirect('/redirect/3.html', urls.EMPTY_PAGE);
      const response = await page.goto(urls.PREFIX + '/redirect/1.html');
      expect(response.ok).toBe(true);
      expect(response.url).toBe(urls.EMPTY_PAGE);
    });
    it('should wait for network idle to succeed navigation', async({page, urls, server}) => {
      let responses = [];
      ...
      const navigationPromise = page.goto(urls.PREFIX + '/networkidle.html', {
    ...
    it('should not leak listeners during navigation', async({page, urls}) => {
      let warning = null;
    ...
    it('should not leak listeners during bad navigation', async({page, urls}) => {
      let warning = null;
      const warningHandler = w => warning = w;
      process.on('warning', warningHandler);
      for (let i = 0; i < 20; ++i)
    ...
    it('should navigate to dataURL and fire dataURL requests', async({page, urls}) => {
      const requests = [];
    ...
    it('should navigate to URL with hash and fire requests without hash', async({page, urls}) => {
      const requests = [];
      page.on('request', request => requests.push(request));
      const response = await page.goto(urls.EMPTY_PAGE + '#hash');
      expect(response.status).toBe(200);
      expect(response.url).toBe(urls.EMPTY_PAGE);
      expect(requests.length).toBe(1);
      expect(requests[0].url).toBe(urls.EMPTY_PAGE);
    });
  });
  ...
  describe('Page.waitForNavigation', function() {
    it('should work', async({page, urls}) => {
      ...
      const [result] = await Promise.all([
        ...
        page.evaluate(url => window.location.href = url, urls.PREFIX + '/grid.html')
      ]);
      const response = await result;
      expect(response.ok).toBe(true);
      expect(response.url).toContain('grid.html');
    });
    it('should work with both domcontentloaded and load', async({page, urls, server}) => {
      let response = null;
      server.setRoute('/one-style.css', (req, res) => response = res);
      page.goto(urls.PREFIX + '/one-style.html');
  ...
  describe('Page.goBack', function() {
    it('should work', async({page, urls}) => {
      ...
      expect(response.ok).toBe(true);
      expect(response.url).toContain(urls.EMPTY_PAGE);
  ...
  describe('Page.exposeFunction', function() {
    ...
    it('should survive navigation', async({page, urls}) => {
      await page.exposeFunction('compute', function(a, b) {
        return a * b;
      });
    ...
    it('should await returned promise', async({page, urls}) => {
      await page.exposeFunction('compute', function(a, b) {
    ...
    it('should work on frames', async({page, urls}) => {
      await page.exposeFunction('compute', function(a, b) {
        return Promise.resolve(a * b);
      });
    ...
    it('should work on frames before navigation', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/frames/nested-frames.html');
      await page.exposeFunction('compute', function(a, b) {
        return Promise.resolve(a * b);
      });
  ...
  describe('Page.setRequestInterception', function() {
    it('should intercept', async({page, urls}) => {
    ...
    it('should stop intercepting', async({page, urls}) => {
      await page.setRequestInterception(true);
    ...
    it('should show custom HTTP headers', async({page, urls}) => {
    ...
    it('should be abortable', async({page, urls}) => {
      await page.setRequestInterception(true);
      page.on('request', request => {
      ...
      const response = await page.goto(urls.PREFIX + '/one-style.html');
      expect(response.ok).toBe(true);
      expect(response.request().failure()).toBe(null);
      expect(failedRequests).toBe(1);
    });
    it('should be abortable with custom error codes', async({page, urls}) => {
      await page.setRequestInterception(true);
      page.on('request', request => {
      ...
      await page.goto(urls.EMPTY_PAGE).catch(e => {});
      expect(failedRequest).toBeTruthy();
      expect(failedRequest.failure().errorText).toBe('net::ERR_INTERNET_DISCONNECTED');
    });
    it('should amend HTTP headers', async({page, urls, server}) => {
      await page.setRequestInterception(true);
      page.on('request', request => {
    ...
    it('should fail navigation when aborting main resource', async({page, urls}) => {
      await page.setRequestInterception(true);
    ...
    it('should work with redirects', async({page, urls, server}) => {
      await page.setRequestInterception(true);
      const requests = [];
      page.on('request', request => {
        request.continue();
      ...
      const response = await page.goto(urls.PREFIX + '/non-existing-page.html');
      expect(response.status).toBe(200);
      expect(response.url).toContain('empty.html');
      expect(requests.length).toBe(5);
      expect(requests[2].resourceType).toBe('document');
    });
    it('should be able to abort redirects', async({page, urls, server}) => {
      await page.setRequestInterception(true);
    ...
    it('should work with equal requests', async({page, urls, server}) => {
      await page.goto(urls.EMPTY_PAGE);
      let responseCount = 1;
    ...
    it('should abort data URLs', async({page, urls}) => {
      await page.setRequestInterception(true);
      page.on('request', request => {
        request.abort();
      });
    ...
    it('should navigate to URL with hash and and fire requests without hash', async({page, urls}) => {
      await page.setRequestInterception(true);
      const requests = [];
      page.on('request', request => {
        requests.push(request);
    ...
    it('should work with encoded URLs', async({page, urls}) => {
      // The requestWillBeSent will report encoded URL, whereas interception will
      // report URL as-is. @see crbug.com/759388
      await page.setRequestInterception(true);
      page.on('request', request => request.continue());
      const response = await page.goto(urls.PREFIX + '/some nonexisting page');
      expect(response.status).toBe(404);
    });
    it('should work with badly encoded URLs', async({page, urls, server}) => {
      await page.setRequestInterception(true);
      server.setRoute('/malformed?rnd=%911', (req, res) => res.end());
      page.on('request', request => request.continue());
      const response = await page.goto(urls.PREFIX + '/malformed?rnd=%911');
      expect(response.status).toBe(200);
    });
    it('should work with encoded URLs - 2', async({page, urls}) => {
      ...
      const response = await page.goto(`data:text/html,<link rel="stylesheet" href="${urls.PREFIX}/fonts?helvetica|arial"/>`);
      expect(response.status).toBe(200);
      expect(requests.length).toBe(2);
      expect(requests[1].response().status).toBe(404);
    });
    it('should not throw "Invalid Interception Id" if the request was cancelled', async({page, urls}) => {
      await page.setContent('<iframe></iframe>');
      await page.setRequestInterception(true);
      let request = null;
      page.on('request', async r => request = r);
      page.$eval('iframe', (frame, url) => frame.src = url, urls.EMPTY_PAGE),
    ...
    it('should throw if interception is not enabled', async({page, urls}) => {
      let error = null;
  ...
  describe('Request.respond', function() {
    ...
    it('should allow mocking binary responses', async({page, urls}) => {
      await page.setRequestInterception(true);
      page.on('request', request => {
  ...
  describe('Page.Events.Dialog', function() {
    it('should fire', async({page, urls}) => {
    ...
    it('should allow accepting prompts', async({page, urls}) => {
      page.on('dialog', dialog => {
    ...
    it('should dismiss the prompt', async({page, urls}) => {
      page.on('dialog', dialog => {
        dialog.dismiss();
      });
  ...
  describe('Page.Events.PageError', function() {
    it('should fire', async({page, urls}) => {
      ...
      page.goto(urls.PREFIX + '/error.html');
  ...
  describe('Frame Management', function() {
    const FrameUtils = require('./frame-utils');
    it('should handle nested frames', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/frames/nested-frames.html');
      expect(FrameUtils.dumpFrames(page.mainFrame())).toBeGolden('nested-frames.txt');
    });
    it('should send events when frames are manipulated dynamically', async({page, urls}) => {
      await page.goto(urls.EMPTY_PAGE);
    ...
    it('should persist mainFrame on cross-process navigation', async({page, urls}) => {
      await page.goto(urls.EMPTY_PAGE);
      const mainFrame = page.mainFrame();
      await page.goto(urls.CROSS_PROCESS_PREFIX + '/empty.html');
      expect(page.mainFrame() === mainFrame).toBeTruthy();
    });
    it('should not send attach/detach events for main frame', async({page, urls}) => {
      let hasEvents = false;
    ...
    it('should detach child frames on navigation', async({page, urls}) => {
      let attachedFrames = [];
      let detachedFrames = [];
      let navigatedFrames = [];
      page.on('frameattached', frame => attachedFrames.push(frame));
    ...
    it('should report frame.name()', async({page, urls}) => {
      await FrameUtils.attachFrame(page, 'theFrameId', urls.EMPTY_PAGE);
    ...
    it('should report frame.parent()', async({page, urls}) => {
      await FrameUtils.attachFrame(page, 'frame1', urls.EMPTY_PAGE);
      await FrameUtils.attachFrame(page, 'frame2', urls.EMPTY_PAGE);
  ...
  describe('Page.$', function() {
    it('should query existing element', async({page, urls}) => {
      await page.setContent('<section>test</section>');
      const element = await page.$('section');
      expect(element).toBeTruthy();
    });
    it('should return null for non-existing element', async({page, urls}) => {
  ...
  describe('Page.$$', function() {
    it('should query existing elements', async({page, urls}) => {
    ...
    it('should return empty array if nothing is found', async({page, urls}) => {
      await page.goto(urls.EMPTY_PAGE);
      const elements = await page.$$('div');
  ...
  describe('ElementHandle.boundingBox', function() {
    ...
    it('should return null for invisible elements', async({page, urls}) => {
  ...
  describe('ElementHandle.click', function() {
    it('should work', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/button.html');
      const button = await page.$('button');
      await button.click();
      expect(await page.evaluate(() => result)).toBe('Clicked');
    });
    it('should work for Shadow DOM v1', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/shadow.html');
      const buttonHandle = await page.evaluateHandle(() => button);
      await buttonHandle.click();
      expect(await page.evaluate(() => clicked)).toBe(true);
    });
    it('should work for TextNodes', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/button.html');
    ...
    it('should throw for detached nodes', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/button.html');
      const button = await page.$('button');
    ...
    it('should throw for hidden nodes', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/button.html');
      const button = await page.$('button');
    ...
    it('should throw for recursively hidden nodes', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/button.html');
      const button = await page.$('button');
    ...
    it('should throw for <br> elements', async({page, urls}) => {
  ...
  describe('ElementHandle.hover', function() {
    it('should work', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/scrollable.html');
  ...
  describe('ElementHandle.screenshot', function() {
    ...
    it('should take into account padding and border', async({page, urls}) => {
      await page.setViewport({width: 500, height: 500});
    ...
    it('should scroll element into view', async({page, urls}) => {
      await page.setViewport({width: 500, height: 500});
      await page.setContent(`
        something above
        <style>div.above {
    ...
    it('should work with a rotated element', async({page, urls}) => {
      await page.setViewport({width: 500, height: 500});
    ...
    it('should fail to screenshot a detached element', async({page, urls}) => {
  ...
  describe('ElementHandle.$', function() {
    it('should query existing element', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/playground.html');
  ...
  describe('ElementHandle.$$', function() {
    ...
    it('should return empty array for non-existing elements', async({page, urls}) => {
  ...
  describe('input', function() {
    it('should click the button', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/button.html');
      await page.click('button');
      expect(await page.evaluate(() => result)).toBe('Clicked');
    });

    it('should click on checkbox input and toggle', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/checkbox.html');
    ...
    it('should click on checkbox label and toggle', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/checkbox.html');
      expect(await page.evaluate(() => result.check)).toBe(null);
    ...
    it('should fail to click a missing button', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/button.html');
      let error = null;
    ...
    it('should not hang with touch-enabled viewports', async({page, urls}) => {
      await page.setViewport(iPhone.viewport);
      await page.mouse.down();
      await page.mouse.move(100, 10);
      await page.mouse.up();
    });
    it('should type into the textarea', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/textarea.html');

      const textarea = await page.$('textarea');
      await textarea.type('Type in this text!');
      expect(await page.evaluate(() => result)).toBe('Type in this text!');
    });
    it('should click the button after navigation ', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/button.html');
      await page.click('button');
      await page.goto(urls.PREFIX + '/input/button.html');
      await page.click('button');
    ...
    it('should upload the file', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/fileupload.html');
    ...
    it('should move with the arrow keys', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/textarea.html');
    ...
    it('should send a character with ElementHandle.press', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/textarea.html');
      const textarea = await page.$('textarea');
    ...
    it('should send a character with sendCharacter', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/textarea.html');
    ...
    it('should report shiftKey', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/keyboard.html');
    ...
    it('should report multiple modifiers', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/keyboard.html');
      const keyboard = page.keyboard;
    ...
    it('should send proper codes while typing', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/keyboard.html');
    ...
    it('should send proper codes while typing with shift', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/keyboard.html');
      const keyboard = page.keyboard;
    ...
    it('should not type canceled events', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/textarea.html');
      await page.focus('textarea');
      await page.evaluate(() => {
    ...
    it('keyboard.modifiers()', async({page, urls}) => {
      const keyboard = page.keyboard;
    ...
    it('should resize the textarea', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/textarea.html');
    ...
    it('should scroll and click the button', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/scrollable.html');
    ...
    it('should click a partially obscured button', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/button.html');
      await page.evaluate(() => {
    ...
    it('should select the text with mouse', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/textarea.html');
      await page.focus('textarea');
    ...
    it('should select the text by triple clicking', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/textarea.html');
      await page.focus('textarea');
      const text = 'This is the text that we are going to try to select. Let\'s see how it goes.';
      await page.keyboard.type(text);
    ...
    it('should trigger hover state', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/scrollable.html');
    ...
    it('should fire contextmenu event on right click', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/scrollable.html');
      await page.click('#button-8', {button: 'right'});
      expect(await page.evaluate(() => document.querySelector('#button-8').textContent)).toBe('context menu');
    });
    it('should set modifier keys on click', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/scrollable.html');
    ...
    it('should specify repeat property', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/textarea.html');
      await page.focus('textarea');
    ...
    it('should click links which cause navigation', async({page, urls}) => {
      await page.setContent(`<a href="${urls.EMPTY_PAGE}">empty.html</a>`);
      // This await should not hang.
      await page.click('a');
    });
    it('should tween mouse movement', async({page, urls}) => {
      await page.mouse.move(100, 100);
      await page.evaluate(() => {
        window.result = [];
    ...
    it('should tap the button', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/button.html');
      await page.tap('button');
      expect(await page.evaluate(() => result)).toBe('Clicked');
    });
    xit('should report touches', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/touches.html');
      const button = await page.$('button');
      await button.tap();
      expect(await page.evaluate(() => getResult())).toEqual(['Touchstart: 0', 'Touchend: 0']);
    });
    it('should click the button inside an iframe', async({page, urls}) => {
      await page.goto(urls.EMPTY_PAGE);
      await page.setContent('<div style="width:100px;height:100px">spacer</div>');
      const FrameUtils = require('./frame-utils');
      await FrameUtils.attachFrame(page, 'button-test', urls.PREFIX + '/input/button.html');
      const frame = page.frames()[1];
      const button = await frame.$('button');
      await button.click();
      expect(await frame.evaluate(() => window.result)).toBe('Clicked');
    });
    it('should click the button with deviceScaleFactor set', async({page, urls}) => {
    ...
    it('should type all kinds of characters', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/textarea.html');
      await page.focus('textarea');
    ...
    it('should specify location', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/textarea.html');
      await page.evaluate(() => {
    ...
    it('should throw on unknown keys', async({page, urls}) => {
  ...
  describe('Page.setUserAgent', function() {
    it('should work', async({page, urls, server}) => {
      expect(await page.evaluate(() => navigator.userAgent)).toContain('Mozilla');
      page.setUserAgent('foobar');
      const [request] = await Promise.all([
        server.waitForRequest('/empty.html'),
        page.goto(urls.EMPTY_PAGE),
      ]);
      expect(request.headers['user-agent']).toBe('foobar');
    });
    it('should emulate device user-agent', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/mobile.html');
  ...
  describe('Page.setExtraHTTPHeaders', function() {
    ...
    it('should throw for non-string header values', async({page, urls}) => {
      let error = null;
      try {
  ...
  describe('Page.authenticate', function() {
    it('should work', async({page, urls, server}) => {
      ...
      let response = await page.goto(urls.EMPTY_PAGE);
    ...
    it('should fail if wrong credentials', async({page, urls, server}) => {
    ...
    it('should allow disable authentication', async({page, urls, server}) => {
      // Use unique user/password since Chrome caches credentials per origin.
      ...
      response = await page.goto(urls.CROSS_PROCESS_PREFIX + '/empty.html');
      expect(response.status).toBe(401);
    });
  });
  ...
  describe('Page.setContent', function() {
    ...
    it('should work with doctype', async({page, urls}) => {
      const doctype = '<!DOCTYPE html>';
      await page.setContent(`${doctype}<div>hello</div>`);
      const result = await page.content();
      expect(result).toBe(`${doctype}${expectedOutput}`);
    });
    it('should work with HTML 4 doctype', async({page, urls}) => {
  ...
  describe('Network Events', function() {
    it('Page.Events.Request', async({page, urls}) => {
      const requests = [];
      page.on('request', request => requests.push(request));
      await page.goto(urls.EMPTY_PAGE);
      expect(requests.length).toBe(1);
    ...
    it('Page.Events.Request should report post data', async({page, urls, server}) => {
      await page.goto(urls.EMPTY_PAGE);
    ...
    it('Page.Events.Response', async({page, urls}) => {
      const responses = [];
      page.on('response', response => responses.push(response));
      await page.goto(urls.EMPTY_PAGE);
      expect(responses.length).toBe(1);
      expect(responses[0].url).toBe(urls.EMPTY_PAGE);
      expect(responses[0].status).toBe(200);
      expect(responses[0].ok).toBe(true);
      expect(responses[0].request()).toBeTruthy();
    });
    it('Page.Events.Response should provide body', async({page, urls}) => {
      let response = null;
      page.on('response', r => response = r);
      await page.goto(urls.PREFIX + '/simple.json');
      expect(response).toBeTruthy();
      expect(await response.text()).toBe('{"foo": "bar"}\n');
      expect(await response.json()).toEqual({foo: 'bar'});
    });
    it('Page.Events.Response should not report body unless request is finished', async({page, urls, server}) => {
      await page.goto(urls.EMPTY_PAGE);
    ...
    it('Page.Events.RequestFailed', async({page, urls}) => {
      await page.setRequestInterception(true);
      page.on('request', request => {
        if (request.url.endsWith('css'))
          request.abort();
      ...
      await page.goto(urls.PREFIX + '/one-style.html');
    ...
    it('Page.Events.RequestFinished', async({page, urls}) => {
      const requests = [];
    ...
    it('should fire events in proper order', async({page, urls}) => {
      const events = [];
    ...
    it('should support redirects', async({page, urls, server}) => {
      const events = [];
      ...
      const FOO_URL = urls.PREFIX + '/foo.html';
      ...
      expect(events).toEqual([
        ...
        `GET ${urls.EMPTY_PAGE}`,
        `200 ${urls.EMPTY_PAGE}`,
        `DONE ${urls.EMPTY_PAGE}`
      ]);
    });
  });

  describe('Page.addScriptTag', function() {
    it('should throw an error if no options are provided', async({page, urls}) => {
      let error = null;
      try {
    ...
    it('should work with a url', async({page, urls}) => {
      await page.goto(urls.EMPTY_PAGE);
    ...
    it('should throw an error if loading from url fail', async({page, urls}) => {
      await page.goto(urls.EMPTY_PAGE);
      let error = null;
      try {
    ...
    it('should work with a path', async({page, urls}) => {
      await page.goto(urls.EMPTY_PAGE);
    ...
    it('should include sourcemap when path is provided', async({page, urls}) => {
      await page.goto(urls.EMPTY_PAGE);
    ...
    it('should work with content', async({page, urls}) => {
      await page.goto(urls.EMPTY_PAGE);
  ...
  describe('Page.url', function() {
    it('should work', async({page, urls}) => {
      ...
      await page.goto(urls.EMPTY_PAGE);
      expect(page.url()).toBe(urls.EMPTY_PAGE);
    });
  });

  describe('Page.viewport', function() {
    it('should get the proper viewport size', async({page, urls}) => {
      expect(page.viewport()).toEqual({width: 800, height: 600});
      await page.setViewport({width: 123, height: 456});
      expect(page.viewport()).toEqual({width: 123, height: 456});
    });
    it('should support mobile emulation', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/mobile.html');
    ...
    it('should support touch emulation', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/mobile.html');
    ...
    it('should be detectable by Modernizr', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/detect-touch.html');
      expect(await page.evaluate(() => document.body.textContent.trim())).toBe('NO');
      await page.setViewport(iPhone.viewport);
      await page.goto(urls.PREFIX + '/detect-touch.html');
      expect(await page.evaluate(() => document.body.textContent.trim())).toBe('YES');
    });
    it('should support landscape emulation', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/mobile.html');
  ...
  describe('Page.emulate', function() {
    ...
    it('should support clicking', async({page, urls}) => {
      await page.emulate(iPhone);
      await page.goto(urls.PREFIX + '/input/button.html');
      const button = await page.$('button');
  ...
  describe('Page.emulateMedia', function() {
    ...
    it('should throw in case of bad argument', async({page, urls}) => {
      let error = null;
  ...
  describe('Page.evaluateOnNewDocument', function() {
    it('should evaluate before anything else on the page', async({page, urls}) => {
      await page.evaluateOnNewDocument(function(){
        window.injected = 123;
      });
      await page.goto(urls.PREFIX + '/tamperable.html');
  ...
  (headless ? describe : xdescribe)('Page.pdf', function() {
    it('should be able to save file', async({page, urls}) => {
      const outputFile = __dirname + '/assets/output.pdf';
      await page.pdf({path: outputFile});
      expect(fs.readFileSync(outputFile).byteLength).toBeGreaterThan(0);
      fs.unlinkSync(outputFile);
    });
    it('should default to printing in Letter format', async({page, urls}) => {
    ...
    it('should support setting custom format', async({page, urls}) => {
      const pages = await getPDFPages(await page.pdf({
        format: 'a4'
      }));
      expect(pages.length).toBe(1);
    ...
    it('should support setting paper width and height', async({page, urls}) => {
      const pages = await getPDFPages(await page.pdf({
        width: '10in',
        height: '10in',
      }));
    ...
    it('should print multiple pages', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/grid.html');
    ...
    it('should support page ranges', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/grid.html');
      // Define width and height in CSS pixels.
      const width = 50 * 5 + 1;
      const height = 50 * 5 + 1;
    ...
    it('should throw if format is unknown', async({page, urls}) => {
      let error = null;
      try {
    ...
    it('should throw if units are unknown', async({page, urls}) => {
      let error = null;
      try {
        await getPDFPages(await page.pdf({
          width: '10em',
  ...
  describe('Page.title', function() {
    it('should return the page title', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/button.html');
  ...
  describe('Page.screenshot', function() {
    ...
    it('should clip rect', async({page, urls}) => {
      await page.setViewport({width: 500, height: 500});
      await page.goto(urls.PREFIX + '/grid.html');
      const screenshot = await page.screenshot({
        clip: {
    ...
    it('should work for offscreen clip', async({page, urls}) => {
      await page.setViewport({width: 500, height: 500});
      await page.goto(urls.PREFIX + '/grid.html');
      const screenshot = await page.screenshot({
        clip: {
    ...
    it('should run in parallel', async({page, urls}) => {
      await page.setViewport({width: 500, height: 500});
      await page.goto(urls.PREFIX + '/grid.html');
      const promises = [];
      for (let i = 0; i < 3; ++i) {
    ...
    it('should take fullPage screenshots', async({page, urls}) => {
      await page.setViewport({width: 500, height: 500});
      await page.goto(urls.PREFIX + '/grid.html');
      const screenshot = await page.screenshot({
        fullPage: true
    ...
    it('should run in parallel in multiple pages', async({page, urls, browser}) => {
      const N = 2;
    ...
    it('should allow transparency', async({page, urls}) => {
      await page.setViewport({ width: 100, height: 100 });
      await page.goto(urls.EMPTY_PAGE);
      const screenshot = await page.screenshot({omitBackground: true});
      expect(screenshot).toBeGolden('transparent.png');
    });
    it('should work with odd clip size on Retina displays', async({page, urls}) => {
      const screenshot = await page.screenshot({
        clip: {
          x: 0,
          y: 0,
  ...
  describe('Page.select', function() {
    it('should select single option', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/select.html');
      await page.select('select', 'blue');
      expect(await page.evaluate(() => result.onInput)).toEqual(['blue']);
      expect(await page.evaluate(() => result.onChange)).toEqual(['blue']);
    });
    it('should select multiple options', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/select.html');
    ...
    it('should respect event bubbling', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/select.html');
      await page.select('select', 'blue');
    ...
    it('should throw when element is not a <select>', async({page, urls}) => {
      let error = null;
      await page.goto(urls.PREFIX + '/input/select.html');
      await page.select('body', '').catch(e => error = e);
      expect(error.message).toContain('Element is not a <select> element.');
    });
    it('should return [] on no matched values', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/select.html');
      const result = await page.select('select','42','abc');
      expect(result).toEqual([]);
    });
    it('should return an array of matched values', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/select.html');
      await page.evaluate(() => makeMultiple());
    ...
    it('should return an array of one element when multiple is not set', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/select.html');
      const result = await page.select('select','42','blue','black','magenta');
      expect(result.length).toEqual(1);
    });
    it('should return [] on no values',async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/select.html');
      const result = await page.select('select');
      expect(result).toEqual([]);
    });
    it('should deselect all options when passed no values for a multiple select',async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/select.html');
      await page.evaluate(() => makeMultiple());
    ...
    it('should deselect all options when passed no values for a select without multiple',async({page, urls}) => {
      await page.goto(urls.PREFIX + '/input/select.html');
      await page.select('select','blue','black','magenta');
      await page.select('select');
      expect(await page.$eval('select', select => Array.from(select.options).every(option => !option.selected))).toEqual(true);
    });
    it('should throw if passed in non-strings', async({page, urls}) => {
  ...
  describe('Tracing', function() {
    ...
    it('should output a trace', async({page, urls, outputFile}) => {
    ...
    it('should throw if tracing on two pages', async({page, urls, browser, outputFile}) => {
  ...
  describe('Cookies', function() {
    afterEach(async({page, urls}) => {
      const cookies = await page.cookies(urls.PREFIX + '/grid.html', urls.CROSS_PROCESS_PREFIX);
      for (const cookie of cookies)
        await page.deleteCookie(cookie);
    });
    it('should set and get cookies', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/grid.html');
    ...
    it('should set a cookie with a path', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/grid.html');
      await page.setCookie({
        name: 'gridcookie',
        value: 'GRID',
      ...
      await page.goto(urls.PREFIX + '/empty.html');
      expect(await page.cookies()).toEqual([]);
    ...
    it('should delete a cookie', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/grid.html');
      await page.setCookie({
        name: 'cookie1',
        value: '1'
    ...
    it('should set a cookie on a different domain', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/grid.html');
    ...
    it('should set cookies from a frame', async({page, urls}) => {
      await page.goto(urls.PREFIX + '/grid.html');
      ...
      await page.evaluate(src => {
        ...
        iframe.src = src;
        return promise;
      }, urls.CROSS_PROCESS_PREFIX);
      await page.setCookie({name: '127-cookie', value: 'worst', url: urls.CROSS_PROCESS_PREFIX});
      ...
      expect(await page.cookies(urls.CROSS_PROCESS_PREFIX)).toEqual([{
        name: '127-cookie',
        value: 'worst',
        domain: '127.0.0.1',
        path: '/',
  ...
  describe('Target', function() {
    it('Browser.targets should return all of the targets', async({page, urls, browser}) => {
    ...
    it('Browser.pages should return all of the pages', async({page, urls, browser}) => {
      // The pages will be the testing page and the original newtab page
    ...
    it('should be able to use the default page in the browser', async({page, urls, browser}) => {
      // The pages will be the testing page and the original newtab page
      const allPages = await browser.pages();
    ...
    it('should report when a new page is created and closed', async({page, urls, browser}) => {
      const otherPagePromise = new Promise(fulfill => browser.once('targetcreated', target => fulfill(target.page())));
      await page.evaluate(url => window.open(url), urls.CROSS_PROCESS_PREFIX);
      const otherPage = await otherPagePromise;
      expect(otherPage.url()).toContain(urls.CROSS_PROCESS_PREFIX);
    ...
    it('should report when a service worker is created and destroyed', async({page, urls, browser}) => {
      await page.goto(urls.EMPTY_PAGE);
      ...
      expect((await createdTarget).url()).toBe(urls.PREFIX + '/sw.js');
    ...
    it('should report when a target url changes', async({page, urls, browser}) => {
      await page.goto(urls.EMPTY_PAGE);
      let changedTarget = new Promise(fulfill => browser.once('targetchanged', target => fulfill(target)));
      await page.goto(urls.CROSS_PROCESS_PREFIX + '/');
      expect((await changedTarget).url()).toBe(urls.CROSS_PROCESS_PREFIX + '/');

      changedTarget = new Promise(fulfill => browser.once('targetchanged', target => fulfill(target)));
      await page.goto(urls.EMPTY_PAGE);
      expect((await changedTarget).url()).toBe(urls.EMPTY_PAGE);
    });
    it('should not report uninitialized pages', async({page, urls, browser}) => {
    ...
    it('should not crash while redirecting if original request was missed', async({page, urls, server, browser}) => {
      let serverResponse = null;
      server.setRoute('/one-style.css', (req, res) => serverResponse = res);
      // Open a new page. Use window.open to connect to the page later.
      await Promise.all([
        page.evaluate(url => window.open(url), urls.PREFIX + '/one-style.html'),
...
if (process.env.COVERAGE) {
  describe('COVERAGE', function(){
    ...
    for (const method of coverage.keys()) {
      (disabled.has(method) ? xit : it)(`public api '${method}' should be called`, async({page, urls}) => {
