...
module.exports.addTests = function({testRunner, expect, product}) {
  ...
  describe('Page.waitForFunction', function() {
  ...
  describe('Page.waitForSelector', function() {
    ...
    it('Page.waitForSelector is shortcut for main frame', async({page, server}) => {
      ...
      const eHandle = await watchdog;
      expect(await page.mainFrame().evaluate(e => !!e, eHandle)).toBe(true);
    });
    ...
    it('should run in specified frame', async({page, server}) => {
      ...
      expect(await frame2.evaluate(e => !!e, eHandle)).toBe(true);
    });
    ...
    xit('should survive cross-process navigation', async({page, server}) => {
      let boxFound = false;
  ...
  describe('Page.waitFor', function() {
    ...
    it('should wait for predicate', async({page, server}) => {
      await page.goto(server.EMPTY_PAGE);
      await Promise.all([
        page.waitFor(() => window.location.href.includes('woof')),
        page.goto(server.EMPTY_PAGE + '#woof'),
      ]);
    });
