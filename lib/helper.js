...
class Chain {
  /**
   * @template T
   * @param {Promise<T>} promiseChain
   * @param {Promise} resultPromise
   * @param {string} errorString
   */
  constructor(promiseChain, resultPromise, errorString) {
    this._promiseChain = promiseChain;
    return new Proxy(() => this, {
      apply: (target, ...args) => new Chain(this._promiseChain.then(x => {
      ...
      get: (target, property, receiver) => {
        if (property === 'then' || property === '_promiseChain')
          return Reflect.get(this, property, receiver);
        return new Chain(this._promiseChain.then(x => x[property]), errorString + '.' + property);
      }
    });
  }
  ...
  async then(resolve, reject) {
    return this._promiseChain.then(resolve, reject);
  }
}
