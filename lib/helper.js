...
/**
 * @template T
 * @param {Promise<T>} instancePromise
 * @param {Promise} resultPromise
 * @param {string} errorString
 * @return {Promise<T>}
 */
function Chain(instancePromise, resultPromise, errorString) {
  return new Proxy(() => this, {
    apply: (target, thisArg, args) => {
      let newResultPromise = resultPromise.then(async x => {
        if (typeof x !== 'function')
          throw new Error(`ChainError: ${errorString} is not a function`);
        let instance = await instancePromise;
        let result = x.call(instance, ...args);
        let isChain = (result && result.__isChain__);
        return {result, isChain};
      });

      return Chain(pickInstance(newResultPromise), newResultPromise.then(x => x.result), errorString + '()');
    },

    get: (target, property, receiver) => {
      if (property === 'then')
        return resultPromise.then.bind(resultPromise);
      if (property === '__isChain__')
        return true;
      return Chain(instancePromise, getProperty(property), errorString + '.' + String(property));
    }
  });

  async function pickInstance(newResultPromise) {
    try {
      let {result, isChain} = await newResultPromise;
      if (isChain)
        return result;
    } catch (e) {
      // We will deal with this error later.
    }
