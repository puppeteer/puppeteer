 ...
 */
let methods = new Set();
let runMethods = new Set();
let measuringCoverage = false;
class Helper {
  /**
  ...
  static tracePublicAPI(classType) {
    ...
    if (!debug.enabled && !measuringCoverage)
      return;
    for (let methodName of Reflect.ownKeys(classType.prototype)) {
      ...
      Reflect.set(classType.prototype, methodName, function(...args) {
        ...
        if (debug.enabled)
          debug(callsite);
        runMethods.add(`${className}.${methodName}`);
   ...
   * @return {boolean}
   */
  static wasPublicAPIMethodCovered(method) {
    return runMethods.has(method);
  }

  /**
   * @return {!Array<string>}
   */
  static publicAPIMethods() {
    return Array.from(methods);
  }

  /**
   * @param {boolean} onOnOff
   */
  static toggleCoverage(onOrOff) {
    measuringCoverage = onOrOff;
  }
}
