...
class Downloader {
  /**
   * @param {!Downloader.Options=} options
   */
  constructor(options = {}) {
   ...
   * @return {!Promise}
   */
  async downloadRevision(revision, progressCallback) {
  ...
  /**
   * @param {string} revision
   * @return {!{revision: string, folderPath: string, executablePath: string, url: string, downloaded: boolean}}
   */
  revisionInfo(revision) {
    ...
    let url = downloadURLs[this._platform];
    url = util.format(url, this._downloadHost, revision);
    const downloaded = fs.existsSync(folderPath);
    return {revision, executablePath, folderPath, downloaded, url};
  }
...
/**
 * @typedef {Object} Downloader.Options
