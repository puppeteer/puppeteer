...
function codeForKey(key) {
  if (keys[key])
    return keys[key];
  if (key.length === 1)
    return key.toUpperCase().charCodeAt(key);
  return 0;
}
...
class Keyboard {
   ...
   */
  async type(text) {
    let expression = Helper.evaluationString(checkIfEventWasConsumed);
     ...
     */
    function checkIfEventWasConsumed() {
      let fulfill;
      let promise = new Promise(x => fulfill = x);
      let listener = event => {
        if (!event.isTrusted)
          return;
        setTimeout(() => fulfill(event.defaultPrevented), 0);
