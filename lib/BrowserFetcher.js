...
function archiveName(platform, revision) {
  if (['linux', 'mac'].indexOf(platform) !== -1)
    return `chrome-${platform}`;
  if (['win32', 'win64'].indexOf(platform) !== -1) {
...
class BrowserFetcher {
  ...
  revisionInfo(revision) {
    ...
    if (this._platform === 'mac')
      chromePath = ['Chromium.app', 'Contents', 'MacOS', 'Chromium'];
    ...
    const executablePath = path.join(folderPath, archiveName(this._platform, revision), Array.isArray(chromePath) ? ...chromePath : chromePath);
    const url = downloadURL(this._platform, this._downloadHost, revision);
