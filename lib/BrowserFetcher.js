...
const downloadURLs = {
  ...
  content_shell: {
    linux: '%s/chromium-browser-snapshots/Linux_x64/%d/content-shell.zip',
    mac: '%s/chromium-browser-snapshots/Mac/%d/content-shell.zip',
    win32: '%s/chromium-browser-snapshots/Win/%d/content-shell.zip',
    win64: '%s/chromium-browser-snapshots/Win_x64/%d/content-shell.zip',
  }
};
...
class BrowserFetcher {
  ...
  constructor(options = {}) {
    ...
    this._product = options.product || 'chromium';
    this._downloadURLs = downloadURLs[this._product];
    console.assert(this._downloadURLs, 'Unknown product: ' + this._product);
    if (!this._platform) {
  ...
  canDownload(revision) {
    const url = util.format(this._downloadURLs[this._platform], this._downloadHost, revision);
  ...
  async download(revision, progressCallback) {
    let url = this._downloadURLs[this._platform];
  ...
  async localRevisions() {
    ...
    for (const fileName of fileNames) {
      const revision = this._parseFolderPath(fileName);
      if (revision)
  ...
  revisionInfo(revision) {
    ...
    } else if (this._product === 'content_shell') {
      if (this._platform === 'mac')
  ...
  _getFolderPath(revision) {
    const productPrefix = this._product === 'chromium' ? '' : 'contentshell-';
    return path.join(this._downloadsFolder, productPrefix + this._platform + '-' + revision);
  }
   ...
   * @return {string}
   */
  _parseFolderPath(folderPath) {
    let name = path.basename(folderPath);
    let product = 'chromium';
    if (name.startsWith('contentshell-')) {
      product = 'content_shell';
      name = name.substring('contentshell-'.length);
    }
