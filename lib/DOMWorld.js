...
class WaitTask {
  /**
   * @param {!IsolatedWorld} isolatedWorld
   * @param {Function|string} predicateBody
   * @param {string|number} polling
   * @param {number} timeout
   * @param {!Array<*>} args
   */
  constructor(isolatedWorld, predicateBody, title, polling, timeout, ...args) {
    ...
    this._isolatedWorld = isolatedWorld;
    this._polling = polling;
    this._timeout = timeout;
    this._predicateBody = helper.isString(predicateBody) ? 'return (' + predicateBody + ')' : 'return (' + predicateBody + ')(...args)';
    this._args = args;
    this._runCount = 0;
    isolatedWorld._waitTasks.add(this);
  ...
  async rerun() {
    ...
    let error = null;
    try {
      success = await (await this._isolatedWorld.executionContext()).evaluateHandle(waitForPredicatePageFunction, this._predicateBody, this._polling, this._timeout, ...this._args);
    } catch (e) {
      error = e;
    }
    ...
    if (!error && await this._isolatedWorld.evaluate(s => !s, success).catch(e => true)) {
      await success.dispose();
      return;
    }
  ...
  _cleanup() {
    ...
    this._isolatedWorld._waitTasks.delete(this);
