...
class FrameManager extends EventEmitter {
  ...
  constructor(client, frameTree, page) {
    ...
    this._client.on('Page.close', event => {
      console.log('Page.close detected');
      this.emit(FrameManager.Events.ConnectionClosed)
    });
    this._client.on('Target.closeTarget', event => {
      console.log('Target.closeTarget detected');
      this.emit(FrameManager.Events.ConnectionClosed)
    });
    ...
    this._eventListeners = [
      helper.addEventListener(this._client, Connection.Events.Closed, () => {
        this.emit(FrameManager.Events.ConnectionClosed);
      })
    ];
