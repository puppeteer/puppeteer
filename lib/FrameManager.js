...
class FrameManager extends EventEmitter {
  ...
  _onExecutionContextCreated(contextPayload) {
    const frameId = contextPayload.auxData && contextPayload.auxData['frameId'];
    ...
    if (!frame)
      return;
    if (contextPayload.auxData['isDefault']) {
      frame._setDefaultContext(context);
    } else if (contextPayload.origin.startsWith('chrome-extension://')) {
      frame._extensionContexts.add(context);
      this.emit(FrameManager.Events.ExtensionContextCreated, context);
    }
  }
  ...
  _removeContext(context) {
    ...
    else if (frame._extensionContexts.delete(context))
      this.emit(FrameManager.Events.ExtensionContextDestroyed, context);
  }
...
FrameManager.Events = {
  ...
  ExtensionContextCreated: 'extensioncontextcreated',
  ExtensionContextDestroyed: 'extensioncontextdestroyed',
};
...
class Frame {
  ...
  constructor(client, page, parentFrame, frameId) {
    ...
    /** @type {!Set<!ExecutionContext>} */
    this._extensionContexts = new Set();
   ...
   */
  extensionContexts() {
    return [...this._extensionContexts];
  }
