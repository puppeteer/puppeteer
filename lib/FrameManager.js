...
const {VirtualTimeContinuePolicy, VirtualTimeStartPolicy} = require('./VirtualTimeController');
  ...
  */
  ...
  waitFor(selectorOrFunctionOrTimeout, options = {}, ...args) {
    ...
    if (helper.isNumber(selectorOrFunctionOrTimeout)) {
      if (this._page._virtualTimeController)
        return this._page._virtualTimeController.grantVirtualTimeBudget(selectorOrFunctionOrTimeout, VirtualTimeStartPolicy.START_IMMEDIATELY);
...
class WaitTask {
  ...
  constructor(frame, predicateBody, polling, timeout, ...args) {
    ...
    // timeout on our end.
    this._stopVirtualBeginFramesCallback = null;
  ...
  terminate(error) {
    ...
    if (page._compositorController)
      page._virtualTimeController.cancelRepeatingTask(this);
  }
  ...
  async rerun() {
    ...
    const page = this._frame._page;
    // If virtual time with begin frame control is used then make sure virtual time is ticking (since that drives rAF).
    if (page._compositorController && runCount === 1) {
      page._virtualTimeController.scheduleRepeatingTask(this, page._compositorController._animationBeginFrameInterval, 1000, VirtualTimeStartPolicy.START_IMMEDIATELY);
      page._virtualTimeController.startVirtualTime();
    }
    ...
    // If virtual time with begin frame control is used then stop virtual time from ticking.
    if (page._compositorController) {
      const promise = new Promise(resolve => this._stopVirtualBeginFramesCallback = resolve);
      await promise;
      this._stopVirtualBeginFramesCallback = null;
      page._virtualTimeController.cancelRepeatingTask(this);
    }
  ...
  intervalElapsed(virtualTimeOffset, continueCallback) {
    if (this._stopVirtualBeginFramesCallback) {
      continueCallback(VirtualTimeContinuePolicy.NOT_REQUIRED);
      this._stopVirtualBeginFramesCallback();
    } else {
      continueCallback(VirtualTimeContinuePolicy.CONTINUE_MORE_TIME_NEEDED);
    }
  }
}
