...
const fs = require('fs');
const RemoteElement = require('./RemoteElement');
...
class Frame {
  ...
  async evaluate(pageFunction, ...args) {
    let remoteObject = await this._evaluateToRemoteObject(pageFunction, ...args);
   ...
   * @return {!Promise<(!Object|undefined)>}
   */
  async _evaluateToRemoteObject(pageFunction, ...args) {
   ...
   */
  $(selector) {
    return new RemoteElement(this._client, this._resolveElement(selector), this._mouse);
  }
  ...
  async _resolveElement(selector) {
    let remoteObject = await this._evaluateToRemoteObject(selector => document.querySelector(selector), selector);
