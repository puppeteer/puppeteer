...
class FrameManager extends EventEmitter {
  ...
  constructor(client, frameTree, page) {
    ...
    this._client.on('Log.entryAdded', event => this._onEntryAdded(event.entry));
  ...
  _onFrameAttached(frameId, parentFrameId) {
    ...
    const frame = new Frame(this._client, this._page, parentFrame, frameId, this);
  ...
  _onFrameNavigated(framePayload) {
    ...
    if (isMainFrame) {
      if (frame) {
        ...
        frame = new Frame(this._client, this._page, null, framePayload.id, this);
      }
  ...
  _onEntryAdded(entry) {
    if (entry.source === 'security' && entry.level === 'error')
      this._securityError = new Error(entry.text);
  }
...
class Frame {
   ...
   * @param {string} frameId
   * @param {!FrameManager} manager
   */
  constructor(client, page, parentFrame, frameId, manager) {
    this._client = client;
    this._page = page;
  ...
  async addScriptTag(options) {
    if (typeof options.url === 'string') {
      ...
      try {
        return (await this._evaluateHandle(addScriptUrl, url)).asElement();
      } catch (error) {
    ...
    if (typeof options.path === 'string') {
      ...
      return (await this._evaluateHandle(addScriptContent, contents)).asElement();
    }

    if (typeof options.content === 'string')
      return (await this._evaluateHandle(addScriptContent, options.content)).asElement();
  ...
  async addStyleTag(options) {
    if (typeof options.url === 'string') {
      const url = options.url;
      try {
        return (await this._evaluateHandle(addStyleUrl, url)).asElement();
      } catch (error) {
    ...
    if (typeof options.path === 'string') {
      let contents = await readFileAsync(options.path, 'utf8');
      contents += '/*# sourceURL=' + options.path.replace(/\n/g, '') + '*/';
      return (await this._evaluateHandle(addStyleContent, contents)).asElement();
    }

    if (typeof options.content === 'string')
      return (await this._evaluateHandle(addStyleContent, options.content)).asElement();
  ...
  */
   ...
   */
  async _evaluateHandle(pageFunction, arg) {
    const context = await this._contextPromise;
    this._manager._securityError = null;
    const element = await context.evaluateHandle(pageFunction, arg);
    if (this._manager._securityError)
      throw this._manager._securityError;
    return element;
  }
}
