...
class FrameManager extends EventEmitter {
  ...
  _onFrameAttached(frameId, parentFrameId) {
    ...
    const frame = new Frame(this._client, parentFrame, frameId, this._page);
  ...
  _onFrameNavigated(framePayload) {
    ...
    if (isMainFrame) {
      if (frame) {
        ...
        frame = new Frame(this._client, null, framePayload.id, this._page);
      }
...
class Frame {
   ...
   * @param {!Puppeteer.Page} page
   */
  constructor(client, parentFrame, frameId, page) {
    this._client = client;
  ...
  */
  ...
  async waitForNavigation(options = {}) {
    const timeout = typeof options.timeout === 'number' ? options.timeout : this._page._defaultNavigationTimeout;
    const NavigatorWatcher = require('./NavigatorWatcher');
    const watcher = new NavigatorWatcher(this._page._frameManager, this, timeout, options);

    const responses = new Map();
    const listener = helper.addEventListener(this._page._networkManager, NetworkManager.Events.Response, response => responses.set(response.url(), response));
    const error = await watcher.navigationPromise();
    helper.removeEventListeners([listener]);
    if (error)
      throw error;
    return responses.get(this.url()) || null;
  }
