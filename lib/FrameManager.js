...
class Frame {
  ...
  */
  ...
  async _evaluateHandle(pageFunction, arg) {
    const context = await this._contextPromise;
    this._page._cspLogEntry = null;
    const element = await context.evaluateHandle(pageFunction, arg);
    if (this._page._cspLogEntry) {
      const match = this._page._cspLogEntry.text.match(/\('sha256-([^']+)'\)/);
      const hash = match ? match[1] : null;
      if (this._page._cspLogEntry.url === arg || hash === helper.sha256(arg))
        throw new Error(this._page._cspLogEntry.text);
    }
    return element;
  }
}
