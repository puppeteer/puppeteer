...
const {ExecutionContext, ObjectHandle} = require('./ExecutionContext');
...
class FrameManager extends EventEmitter {
  ...
  _onExecutionContextCreated(contextPayload) {
    const context = new ExecutionContext(this._client, contextPayload.id, this.createObjectHandle.bind(this, contextPayload.id));
   ...
   * @return {!ObjectHandle}
   */
  createObjectHandle(contextId, remoteObject) {
    ...
    return new ObjectHandle(context, this._client, remoteObject);
  }
...
class Frame {
   ...
   */
  async $(selector) {
    const handle = await this._context.object(selector => document.querySelector(selector), selector);
   ...
   * @param {function()|string} pageFunction
   * @param {!Array<*>} args
   * @return {!Promise<(!ObjectHandle|!ElementHandle)>}
   */
  async object(pageFunction, ...args) {
    return this._context.object(pageFunction, ...args);
  }
   ...
   */
  async $$(selector) {
    const arrayHandle = await this._context.object(selector => document.querySelectorAll(selector), selector);
    const elements = /* @type {!Array<!ElementHandle>} */(await arrayHandle.asArray());
