...
class Browser {
  ...
  async newPage() {
    ...
    const {sessionId} = await this._connection.send('Target.attachToTarget', {targetId});
    const client = this._connection.createSession(targetId, sessionId);
  ...
  async _launchChrome() {
    ...
    var {port, browserTargetId} = await waitForRemoteDebuggingPort(this._chromeProcess);
