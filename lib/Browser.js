...
class Browser extends EventEmitter {
   ...
   */
  async _targetForId(targetId) {
    const target = this._targets.get(targetId);
    if (!target || !(await target._initializedPromise))
      return null;
    return target;
  }
  ...
  async newPage() {
    ...
    const target = await this._targetForId(targetId);
    console.assert(target, 'Failed to create target for page');
   ...
   */
  targets() {
    return Array.from(this._targets.values()).filter(target => target._wasInitialized);
  }
...
class Target {
  ...
  constructor(browser, targetInfo) {
    ...
    this._initializedPromise = new Promise(fulfill => this._initialized = fulfill);

    this._targetInfoChanged(targetInfo);
  }
  ...
  _targetInfoChanged(targetInfo) {
    ...
    if (!this._wasInitialized && (this._targetInfo.type !== 'page' || this._targetInfo.url !== '')) {
