...
class RemoteElement {
   ...
   * @param {!Promise<?Object>} remoteObjectPromise
   * @param {!Mouse} mouse
   */
  constructor(client, remoteObjectPromise, mouse) {
    this._client = client;
    this._remoteObjectPromise = remoteObjectPromise;
    this._mouse = mouse;
  }
  ...
  async eval(pageFunction, ...args) {
    let { exceptionDetails, result: remoteObject }  = await this._client.send('Runtime.callFunctionOn', {
      objectId: (await this._remoteObjectPromise).objectId,
   ...
   */
  release() {
    let promise = helper.releaseObject(this._client, this._remoteObjectPromise);
    this._remoteObjectPromise = Promise.reject(new Error('Element was released.'));
    return promise;
  }
}
