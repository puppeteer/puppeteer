...
function getWSEndpoint(browserUrl) {
  ...
  function onGetWsEndpointDone(data) {
    try {
  ...
  function onGetWsEndpointError(err) {
  ...
  const request = http.request(requestOptions, res => {
    ...
    if (res.statusCode !== 200) {
      ...
      res.resume();
      onGetWsEndpointError(res.statusCode);
      return;
    }
    res.setEncoding('utf8');
    res.on('data', chunk => data += chunk);
    res.on('end', () => onGetWsEndpointDone(data));
  });

  request.on('error', onGetWsEndpointError);
  request.end();
