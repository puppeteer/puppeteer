...
class InfiniteListenersEmitter extends EventEmitter {
  /**
  ...
  constructor(emitter) {
    ...
    const reemitHandlers = new Map();
    this.on('removeListener', eventName => {
      if (this.listenerCount(eventName) !== 0)
        return;
      const reemit = reemitHandlers.get(eventName);
      emitter.removeListener(eventName, reemit);
      reemitHandlers.delete(eventName);
    });
    this.on('newListener', eventName => {
      if (this.listenerCount(eventName) !== 0)
        return;
      const reemit = (...args) => this.emit(eventName, ...args);
      emitter.on(eventName, reemit);
      reemitHandlers.set(eventName, reemit);
    });
  }
}

const processEmitter = new InfiniteListenersEmitter(process);
...
class Launcher {
  ...
  static async launch(options) {
    ...
    const listeners = [ helper.addEventListener(processEmitter, 'exit', killChrome) ];
    if (options.handleSIGINT !== false)
      listeners.push(helper.addEventListener(processEmitter, 'SIGINT', killChrome));
