...
class Launcher {
  ...
  static async launch(options) {
    ...
    const chromeProcessErrorListeners = [];
    let onChromeProcessError;
    let chromeProcessStartFail = false;

    const chromeProcessErrorPromise = new Promise((resolve, reject) => {
      onChromeProcessError = err => {
        const error = new Error(`Error launching Chromium process with executable path "${chromeExecutable}"`);
        chromeProcessStartFail = true;
        reject(error);
      };

      chromeProcessErrorListeners.push(helper.addEventListener(chromeProcess, 'error', onChromeProcessError));
    });
    ...
    try {
      ...
      const browserWSEndpointPromise = waitForWSEndpoint(chromeProcess, options.timeout || 30 * 1000);

      const browserWSEndpoint = await Promise.race([chromeProcessErrorPromise, browserWSEndpointPromise]);
      helper.removeEventListeners(chromeProcessErrorListeners);
    ...
    function killChrome() {
      ...
      if (chromeProcessStartFail) {
        return Promise.resolve();
      } else if (temporaryUserDataDir) {
        // Force kill chrome.
