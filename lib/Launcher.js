  ...
  static async launch(options) {
    ...
    if (options.dumpio) {
      ...
      if (options.dumpOptions) {
        const { file, ignore } = options.dumpOptions;
        ...
        if (ignore) {
          const writeHook = (write, stream, callback) => function(string, encoding, fd) {
            if (callback(string))
              write.apply(stream, arguments);
            return true;
          };
          const filter = input => ignore.every(criteria => !input.includes(criteria));
          process.stderr.write = writeHook(process.stderr.write, process.stderr, filter);
          process.stdout.write = writeHook(process.stdout.write, process.stdout, filter);
        }
      }
    }
 ...
 * @property {!DumpOptions=} dumpOptions
