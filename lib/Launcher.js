...
class Launcher {
  ...
  static async launch(options) {
    ...
    setTimeout(() => {
      if (!chromeClosed) {
        console.warn('Chrome is still open');
        if (dced)
    ...
    const waitForChromeToClose = new Promise((fulfill, reject) => {
      chromeProcess.once('disconnect', () => {
        dced = true;
      });
      chromeProcess.once('close', e => {
        closed = e || true;
      });
    ...
    chromeProcess['logdata'] = '';
    chromeProcess.stderr.on('data', data => chromeProcess['logdata'] += data.toString('utf8'));
    /** @type {?Connection} */
    let connection = null;
    try {
    ...
    function gracefullyCloseChrome() {
      ...
      if (temporaryUserDataDir) {
        ...
        const timeout = setTimeout(() => console.warn('browser.close timed out'), 2000);
        // Attempt to close chrome gracefully
        connection.send('Browser.close').catch(error => {
          console.warn('error closing chrome', error);
          debugError(error);
          killChrome();
        }).then(() => {
