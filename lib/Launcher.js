...
class Launcher {
  ...
  static async launch(options) {
    ...
    const stdio = [];
    if (options.dumpio)
      stdio.push('inherit', 'inherit', 'inherit');
    else
      stdio.push('pipe', 'pipe', 'pipe');
    if (options.appMode)
    ...
    try {
      ...
      if (!options.appMode) {
        ...
        connection = Connection.createForPipe(chromeProcess, connectionDelay);
      }
  ...
  static async connect(options = {}) {
    const connection = await Connection.createForWebSocket(options.browserWSEndpoint);
