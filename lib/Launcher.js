...
class Launcher {
  ...
  static async launch(options) {
    ...
    try {
      ...
      const commandTimeout = options.commandTimeout || 0;
      if (!usePipe) {
        ...
        connection = await Connection.createForWebSocket(browserWSEndpoint, connectionDelay, commandTimeout);
      } else {
        connection = Connection.createForPipe(/** @type {!NodeJS.WritableStream} */(chromeProcess.stdio[3]), /** @type {!NodeJS.ReadableStream} */ (chromeProcess.stdio[4]), connectionDelay, commandTimeout);
      }
  ...
  static async connect(options = {}) {
    const connectionDelay = options.slowMo || 0;
    const commandTimeout = options.commandTimeout || 0;
    const connection = await Connection.createForWebSocket(options.browserWSEndpoint, connectionDelay, commandTimeout);
