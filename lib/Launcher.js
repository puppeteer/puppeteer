...
class Launcher {
  ...
  static async launch(options) {
    ...
    const listeners = [ addExitEventListener('exit', forceKillChrome) ];
    if (options.handleSIGINT !== false)
      listeners.push(addExitEventListener('SIGINT', forceKillChrome));
    if (options.handleSIGTERM !== false)
      listeners.push(addExitEventListener('SIGTERM', killChrome));
    if (options.handleSIGHUP !== false)
      listeners.push(addExitEventListener('SIGHUP', killChrome));
    /** @type {?Connection} */
    let connection = null;
    try {
 ...
 */
function addExitEventListener(eventName, handler) {
  if (!exitListener) {
    exitListener = new EventEmitter();
    exitListener.setMaxListeners(Infinity);
    const processListeners = [ helper.addEventListener(process, 'exit', emit('exit')) ];
    processListeners.push(helper.addEventListener(process, 'SIGINT', emit('SIGINT')));
    processListeners.push(helper.addEventListener(process, 'SIGTERM', emit('SIGTERM')));
    processListeners.push(helper.addEventListener(process, 'SIGHUP', emit('SIGHUP')));

    function emit(eventName) {
      return function() {
        helper.removeEventListeners(processListeners);
        exitListener.emit(eventName);
      }
    }
  }
  return helper.addEventListener(exitListener, eventName, handler);
}
