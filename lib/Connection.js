...
class Connection extends EventEmitter {
  ...
  _onClose() {
    this.emit(Connection.Events.Closed);
...
class CDPSession extends EventEmitter {
  ...
  constructor(connection, targetType, sessionId) {
    ...
    this._eventListeners = [
      helper.addEventListener(this._connection, Connection.Events.Closed, () => {
        this.emit(Connection.Events.Closed);
      })
    ];
    this._connection.on('Target.closeTarget', () => {
      console.log('target event noticed on client level');
      this.emit(Connection.Events.Closed);
    });
  }
