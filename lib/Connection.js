...
class Connection extends EventEmitter {
   ...
   * @return {!Connection}
   */
  static createForPipe(process, delay = 0) {
    return new Connection('', new Pipe(process), delay);
  }
...
class Pipe extends EventEmitter {
  ...
  constructor(process) {
    super();
    this._pipeWrite = process.stdio[3];
    var pipeRead = process.stdio[4];
  ...
  _dispatch(buffer) {
    ...
    let message = this._pendingMessage + buffer.toString(undefined, 0, end);
