...
class Connection extends EventEmitter {
   ...
   */
  static async createForWebSocket(url, delay = 0, timeout = 0) {
    return new Promise((resolve, reject) => {
      const ws = new WebSocket(url, {perMessageDeflate: false});
      ws.on('open', () => resolve(new Connection(url, ws, delay, timeout)));
      ws.on('error', reject);
    });
  }
   ...
   * @return {!Connection}
   */
  static createForPipe(pipeWrite, pipeRead, delay = 0, timeout = 0) {
    return new Connection('', new Pipe(pipeWrite, pipeRead), delay, timeout);
  }
   ...
   * @param {number=} delay
   */
  constructor(url, transport, delay = 0, timeout = 0) {
    super();
    this._url = url;
    this._lastId = 0;
    /** @type {!Map<number, {resolve: function, reject: function, error: !Error, method: string, timeoutId: NodeJS.Timer}>}*/
      ...
      if (this._timeout > 0) {
        timeoutId = setTimeout(() => {
          if (this._callbacks.get(id)) {
            ...
            reject(`Timeout error (${method}): no response recieved within ${this._timeout}`);
          }
        }, this._timeout);
      }
      this._callbacks.set(id, {resolve, reject, error: new Error(), method, timeoutId});
    });
  }
  ...
  async createSession(targetId) {
    ...
    const session = new CDPSession(this, targetId, sessionId, this._timeout);
...
class CDPSession extends EventEmitter {
   ...
   */
  constructor(connection, targetId, sessionId, timeout = 0) {
    super();
    this._lastId = 0;
    /** @type {!Map<number, {resolve: function, reject: function, error: !Error, method: string, timeoutId: NodeJS.Timer}>}*/
    this._callbacks = new Map();
    ...
    return new Promise((resolve, reject) => {
      ...
      if (this._timeout > 0) {
        timeoutId = setTimeout(() => {
          if (this._callbacks.get(id)) {
            this._callbacks.delete(id);
            reject(`Timeout error (${method}): no response received within ${this._timeout}`);
          }
        }, this._timeout);
      }
      this._callbacks.set(id, {resolve, reject, error: new Error(), method, timeoutId});
