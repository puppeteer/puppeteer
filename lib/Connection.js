...
class Connection extends EventEmitter {
  ...
  static async create(url, delay = 0) {
    return new Promise((resolve, reject) => {
      ...
      ws.on('open', async() => {
        const connection = new Connection(url, ws, delay);
        await connection._initializeMajorVersion();
        resolve(connection);
      });
      ws.on('error', reject);
    });
  ...
  async _initializeMajorVersion() {
    const result = await this.send('Browser.getVersion');
    this._majorVersion = parseInt(result.product.split('/')[1].split('.')[0], 10);
  }
...
class Session extends EventEmitter {
  ...
  /**
   * @param {number} otherVersion
   * @return {boolean}
   */
  chromeVersionIsAtLeast(otherVersion) {
    return this._connection._majorVersion >= otherVersion;
  }
