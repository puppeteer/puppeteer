...
class Page extends EventEmitter {
  ...
  async waitForRequest(url, options = {}) {
    ...
    return helper.waitForEvent(this._networkManager, NetworkManager.Events.Request, request => {
      if (method && !method.includes(request._method)) return false;

      return url === request.url() || (url instanceof RegExp && url.test(request.url()));
    }, timeout);
  }
  /**
   * @param {!Object=} options
  ...
  async waitForResponse(url, options = {}) {
    ...
    return helper.waitForEvent(this._networkManager, NetworkManager.Events.Response, response => {
      if (method && !method.includes(response.request()._method)) return false;
      if (statusCode && !statusCode.includes(response.status())) return false;

      return url === response.url() || (url instanceof RegExp && url.test(response.url()));
    }, timeout);
  }
