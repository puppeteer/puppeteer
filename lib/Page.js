...
class Page extends EventEmitter {
  ...
  /**
   * @param {!{format: "png" | "jpeg", quality: number, maxWidth: number, maxHeight: number, everyNthFrame: number}=} options
   * @return {!Promise}
   */
  async startScreencast(options) {
    if (options.format) console.assert(options.format === 'png' || options.format === 'jpeg', 'Unknown options.format value: ' + options.format);
    if (options.quality) {
  ...
  async screencastFrameAck(sessionId) {
    console.assert(Number.isInteger(sessionId), 'Expected sessionId to be an integer');
