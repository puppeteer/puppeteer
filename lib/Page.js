...
class Page extends EventEmitter {
  ...
  constructor(client, target, frameTree, ignoreHTTPSErrors, screenshotTaskQueue) {
    ...
    this._defaultNetworkTimeout = 30000;
  ...
  async waitForRequest(url, options = {}) {
    ...
    const timeout = typeof options.timeout === 'number' ? options.timeout : this._defaultNetworkTimeout;
    const watcher = new NetworkWatcher(this._networkManager, 'request', url, timeout, options);

    const listener = helper.addEventListener(this._networkManager, NetworkManager.Events.Request, request => {
      if (request.url() === url)
        return matchedRequest = request;

      if (url instanceof RegExp && url.test(request.url()))
        return matchedRequest = request;

      return;
    });

    const error = await watcher.networkPromise();
    helper.removeEventListeners([listener]);
  ...
  async waitForResponse(url, options = {}) {
    ...
    const timeout = typeof options.timeout === 'number' ? options.timeout : this._defaultNetworkTimeout;
    const watcher = new NetworkWatcher(this._networkManager, 'response', url, timeout, options);

    const listener = helper.addEventListener(this._networkManager, NetworkManager.Events.Response, response => {
      if (response.url() === url)
        return matchedRequest = response;

      if (url instanceof RegExp && url.test(response.url()))
        return matchedRequest = response;
