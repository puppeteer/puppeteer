  ...
  async waitForRequest(url, options = {}) {
    ...
    return helper.waitForEvent(this._networkManager, NetworkManager.Events.Request, request => {
      if (url === request.url() || (url instanceof RegExp && url.test(request.url())))
        return method ? method.includes(request.method()) : true;
      return false;
    }, timeout);
  }
  ...
  async waitForResponse(url, options = {}) {
    ...
    return helper.waitForEvent(this._networkManager, NetworkManager.Events.Response, response => {
      if (url === response.url() || (url instanceof RegExp && url.test(response.url())))
        return (method ? method.includes(response.request().method()) : true) && (statusCode ? statusCode.includes(response.status()) : true);
      return false;
    }, timeout);
  }
