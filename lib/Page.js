...
class Page extends EventEmitter {
  ...
  async goto(url, options) {
    ...
    if (error)
      throw error;
    let unreachableURL = this._frameManager.unreachableMainFrameURL();
    if (unreachableURL) {
      const request = requests.get(unreachableURL) || null;
      const errorText = request ? request.failure().errorText : '';
      throw new Error('Failed to navigate: ' + url + (errorText ? ' ' + errorText : ''));
    }
