...
const CompositorController = require('./CompositorController');
const {VirtualTimeController} = require('./VirtualTimeController');
...
class Page extends EventEmitter {
   ...
   * @return {!Promise<!Page>}
   */
  static async create(client, target, ignoreHTTPSErrors, appMode, screenshotTaskQueue, deterministic) {

    await client.send('Page.enable');
    const {frameTree} = await client.send('Page.getFrameTree');
    const page = new Page(client, target, frameTree, ignoreHTTPSErrors, screenshotTaskQueue, deterministic);
   ...
   * @param {Object} deterministic
   */
  constructor(client, target, frameTree, ignoreHTTPSErrors, screenshotTaskQueue, deterministic) {
    super();
    this._client = client;
    this._target = target;
    ...
    this._virtualTimeController = null;
    this._compositorController = null;

    if (deterministic) {
      this._virtualTimeController = new VirtualTimeController(client, deterministic);
      this._compositorController = new CompositorController(client, this._virtualTimeController, deterministic);
    }
    /** @type {!Map<string, Function>} */
    this._pageBindings = new Map();
    this._ignoreHTTPSErrors = ignoreHTTPSErrors;
    this._coverage = new Coverage(client);
    this._defaultNavigationTimeout = deterministic ? 5000 : 30000;
  ...
  _onTargetCrashed() {
    if (this._virtualTimeController)
      this._virtualTimeController.shutdown();
  ...
  async goto(url, options = {}) {
    ...
    let virtualTimeStartedCallback = null;
    const virtualTimeStarted = this._virtualTimeController ? new Promise(resolve => virtualTimeStartedCallback = resolve) : null;
    const watcher = new NavigatorWatcher(this._frameManager, mainFrame, timeout, Object.assign({virtualTimeStartedCallback}, options));
    const navigationPromise = watcher.navigationPromise();
    let error = await Promise.race([
      navigate(this._client, url, referrer, virtualTimeStarted),
      navigationPromise,
    ]);
    if (!error)
     ...
     * @param {Promise=} opt_virtualTimeStarted
     * @return {!Promise<?Error>}
     */
    async function navigate(client, url, referrer, opt_virtualTimeStarted) {
      try {
        // If virtual time is being used, wait for it to initialize before we navigate.
        if (opt_virtualTimeStarted)
          await opt_virtualTimeStarted;
  ...
  async setViewport(viewport) {
    if (this._compositorController)
      await this._compositorController.sendFullBeginFrame();
  ...
  async _screenshotTask(format, options) {
    ...
    let result;
    if (this._compositorController) {
      await this._compositorController.waitUntilIdle();
      // TODO(alexclarke): Support |clip|.
      result = {data: await this._compositorController.captureScreenshot(format, options.quality)};
    } else {
      result = await this._client.send('Page.captureScreenshot', { format, quality: options.quality, clip });
    }
