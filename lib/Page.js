...
const {helper, debugError, assert, DisposablePromise} = require('./helper');
...
class Page extends EventEmitter {
  ...
  _abortOnDisconnect() {
    let reject;
    const promise = new Promise((res, rej) => reject = rej);
    const listener = helper.addEventListener(this._client, Events.CDPSession.Disconnected, event => reject(new Error('Target closed')));
    return new DisposablePromise(promise, () => helper.removeEventListeners([listener]));
  }
  ...
  async waitForRequest(urlOrPredicate, options = {}) {
    ...
    return helper.waitForEvent(this._frameManager.networkManager(), Events.NetworkManager.Request, request => {
      ...
      return false;
    }, timeout, this._abortOnDisconnect());
  }
