...
class Page extends EventEmitter {
  ...
  constructor(client, frameManager, networkManager, screenDPI) {
    ...
    this._mouse = new Input.Mouse(this._client, this._screenDPI);
   ...
   * @param {!Promise}
   */
  async _positionOfNode(selector) {
  ...
  async click(selector) {
    let {x, y} = await this._positionOfNode(selector);
    this._mouse.move(x, y);
    await this._mouse.click();
  }
