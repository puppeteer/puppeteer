...
class Page extends EventEmitter {
  ...
  // codes from: https://github.com/GoogleChrome/puppeteer/issues/537
  async waitForXpath(selector, options = { polling: 'mutation' }) {
	return this.waitForFunction(selector => {
      return null !== document.evaluate(selector, document, null, XPathResult.ANY_TYPE, null);
	}, options, selector);
  }

  async $XPath(selector) {
	const remoteObject = await this.evaluate(selector => {
      return document.evaluate(selector, document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue.textContent;
	}, selector);

	console.log("remote obj: " + Object.getOwnPropertyNames(remoteObject))
	console.log("remote obj: " + remoteObject)
	console.log(remoteObject.subtype)
