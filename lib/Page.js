...
class Page extends EventEmitter {
  ...
  async screenshot(options) {
    ...
    else if (typeof options.clip === 'string') {
      console.assert(options.clip.length > 0, 'Expected options.clip as a string to not be empty');
    }
  ...
  async _screenshotTask(format, options) {
    ...
    if (typeof options.clip === 'string') {
      let handle = await this.$(options.clip);
      console.assert(handle, 'No node found for selector: ' + options.clip);
      const boxModel = await this._client.send('DOM.getBoxModel', { objectId: handle._remoteObject.objectId });
      options.clip = {
