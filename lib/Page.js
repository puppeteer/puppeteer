...
class Page extends EventEmitter {
  ...
  async waitForNavigation(options = {}) {
    const frame = options.frame || this._frameManager.mainFrame();
    const timeout = typeof options.timeout === 'number' ? options.timeout : this._defaultNavigationTimeout;
    const watcher = new NavigatorWatcher(this._frameManager, frame, timeout, options);
    ...
    if (error)
      throw error;
    return responses.get(frame.url()) || null;
  }
