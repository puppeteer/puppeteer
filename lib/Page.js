...
class Page extends EventEmitter {
  ...
  constructor(client, target, frameTree, ignoreHTTPSErrors, screenshotTaskQueue) {
    ...
    this._frameManager.on(FrameManager.Events.ExtensionContextCreated, event => this.emit(Page.Events.ExtensionContextCreated, event));
    this._frameManager.on(FrameManager.Events.ExtensionContextDestroyed, event => this.emit(Page.Events.ExtensionContextDestroyed, event));
  ...
  /**
   * @return {!Array<!Puppeteer.ExecutionContext>}
   */
  extensionContexts() {
    const flattened = [];
    for (const frame of this._frameManager.frames())
      flattened.push(...frame.extensionContexts());
    return flattened;
  }
