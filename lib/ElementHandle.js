...
class ElementHandle extends JSHandle {
  ...
  async _visibleCenter() {
    ...
    return {
      x: box.border.x + box.border.width / 2,
      y: box.border.y + box.border.height / 2
    };
  }
  ...
  /**
   * @return {!Promise<?{border: object, content: object, margin: object, padding: object}>}
   */
  async boxModel() {
    ...
    const calculateBox = function(quad)  {
    ...
    return {
      border: calculateBox(result.model.border),
      content: calculateBox(result.model.content),
      margin: calculateBox(result.model.margin),
      padding: calculateBox(result.model.padding)
    };
  }
  ...
  async screenshot(options = {}) {
    ...
    const boundingBox = await this.boxModel();
    if (!boundingBox)
      throw new Error('Node is not visible');
    const clip = Object.assign({}, boundingBox.border);
    clip.x += pageX;
    clip.y += pageY;
