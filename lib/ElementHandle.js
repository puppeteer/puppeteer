...
class ElementHandle extends JSHandle {
  ...
  async screenshot(options = {}) {
    ...
    if (boundingBox.width > viewport.width || boundingBox.height > viewport.height) {
      ...
      await this._page._emulationManager.emulateViewport(this._client, newViewport);

      needsViewportReset = true;
    }

    await this.executionContext().evaluate(function(el) {
      const parents = [];
      ...
      for (const parent of parents) {
        ...
        const x = elBounds.left - parentBounds.left;
        const y = elBounds.top - parentBounds.top;
      ...
      window.scrollTo(
          bounds2.left + window.scrollX,
          bounds2.top + window.scrollY
      );
    }, this);
    ...
    if (needsViewportReset)
      await this._page._emulationManager.emulateViewport(this._client, viewport);
