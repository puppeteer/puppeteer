      ...
      function setNav(navigatorProperties, webGlProperties) {
        ...
        function overrideNavigator(navigatorProperties) {
          try {
            ...
            extractedPlugins.forEach(function (plugin) {
              var plugin = new Plugin({"name": plugin.name,"filename": plugin.filename,"description":plugin.description,"length":plugin.length})
              plugins.push(plugin);
            });

            extractedMimeTypes.forEach(function (mimeType) {
              var mimeType = new MimeType({"type": mimeType.type,"suffixes": mimeType.suffixes,"description":mimeType.description})
        ...
        function overridePermissions(){
          ...
          return window.navigator.permissions.query = (parameters) => {
            ...
            return parameters.name === 'notifications'  ? Promise.resolve({state: "denied"}) : window.navigator.permissions.__oldquery(parameters)
          };
        }
        ...
        overrideNavigator(navigatorProperties);
        overrideBrokenImage();
        overrideModernzr();
