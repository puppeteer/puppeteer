      ...
      function setNav(navigatorProperties, webGlProperties) {
        ...
        try {
          for (var i in navigatorProperties) {
            ...
            {
              window.navigator[i] = newObject({}, true)
            }
          }
          ...
          // console.log(`EXTRACTED PLUGINS ${JSON.stringify(plugins)}`);
          let mimeTypes = [],
          ...
          // console.log(`EXTRACTED MIMETYPES ${JSON.stringify(mimeTypes)}`);

          navigatorProperties.plugins.forEach(function (value) {
            for (let i = 0; i < value.mimeType.length; i++) {
              ...
              if (mime) {
                let plugin = plugins.find(function (element) {
          ...
          // console.log(`PROCESSED PLUGINS ${JSON.stringify(plugins)}`);


          navigatorProperties.mimeTypes.forEach(function (value) {
            for (let i = 0; i < value.enabledPlugin.length; i++) {
              let pluginName = value.enabledPlugin[i],
                plugin = navigatorProperties.plugins.find(function (element) {
                  return element.name == pluginName;
                });

              if (plugin) {
                let mime = mimeTypes.find(function (element) {
          ...
          // console.log(`PROCESSED MIMES ${JSON.stringify(mimeTypes)}`);

          let masterMimeType = {
            "length": newObject(mimeTypes.length)
          };

          for (let iter = 0; iter < mimeTypes.length; iter++) {
            let mimeType = mimeTypes[iter];
          ...
          Object.defineProperties(window.navigator.mimeTypes, masterMimeType);


          let masterPlugin = {
            "length": newObject(plugins.length)
          };

          for (let iter = 0; iter < plugins.length; iter++) {
          ...
          Object.defineProperties(window.navigator.plugins, masterPlugin);
          //   console.log(`PLUGINS ${JSON.stringify(window.navigator.plugins)}`);
        ...
        /*try {
          ...
          for (let property in oldnavigator) {
            let val = oldnavigator[property];
            if (property == "webdriver") {
              val = false;
            }
            if (typeof (val) == 'function') {
              val.bind(oldnavigator)
            }
            else
            {
              properties[property] = newObject(val)
            }
          }
          ...
          for (let navProperty in navigatorProperties) {
            if (navigatorProperties.hasOwnProperty(navProperty)) {
              ...
              if (navProperty != "plugins" && navProperty != "mimeTypes") {
                properties[navProperty] = newObject(currentProp, false);
              }
              else {
          ...
          Object.defineProperties(newNavigator, properties);
          Object.defineProperty(window, 'navigator', newObject(newNavigator));
