...
class Request {
  ...
  _setFromMemoryCache(fromMemCache) {
    this._fromMemCache = fromMemCache;
  }
...
class Response {
   ...
   * @param {boolean=} fromDiskCache True when the response was served from the disk cache.
   * @param {boolean=} fromServiceWorker True when the response claim from a service worker.
   */
  constructor(client, request, status, headers, fromDiskCache = false, fromServiceWorker = false) {
    this._client = client;
   ...
   */
  fromCache() {
    return this._fromDiskCache || this._request._fromMemCache;
  }
