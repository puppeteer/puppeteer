...
class Request {
  ...
  async mockResponse(response = {}) {
    ...
    responseLines.push(`HTTP/1.1 ${statusCode} ${statusText}`);
    const headers = {};
    if (response.headers) {
      for (const header of Object.keys(response.headers))
        headers[header.toLowerCase()] = response.headers[header];
    }
    if (response.body.length && !('content-length' in headers))
      headers['content-length'] = response.body.length;
    if (response.contentType)
      headers['content-type'] = response.contentType;

    for (const header of Object.keys(headers))
      responseLines.push(header + ': ' + headers[header]);

    responseLines.push('', '');
    let responseBuffer = Buffer.from(responseLines.join('\r\n'), 'utf8');

    if (response.body)
      responseBuffer = Buffer.concat([responseBuffer, response.body]);
