...
class NetworkManager extends EventEmitter {
   ...
   */
  async setBlockedURLs(blockedUrls) {
    this._blockedUrls = [];

    console.assert(Array.isArray(blockedUrls), `Expected blocked urls to be an array of Strings, but "${typeof blockedUrls}" is found.`);

    blockedUrls.forEach(url => {
      console.assert(helper.isString(url), `Expected url "${url}" to be a String, but "${typeof url}" is found`);
      this._blockedUrls.push(url);
    });
    await this._client.send('Network.setBlockedURLs', { urls: this._blockedUrls });
  }
