...
class NetworkManager extends EventEmitter {
  ...
  _onResponseReceived(event) {
    ...
    if (!request)
      return;
    const response = new Response(this._client, request, event.response.status, event.response.headers, event.response.securityDetails);
    request._response = response;
    this.emit(NetworkManager.Events.Response, response);
  }
...
class Response {
   ...
   */
  constructor(client, request, status, headers, securityDetails = null) {
    this._client = client;
    ...
    if (securityDetails) {
      for (const key of ['subjectName', 'issuer', 'validFrom', 'validTo', 'protocol'])
        this._securityDetails[key] = securityDetails[key];
    }
  }
