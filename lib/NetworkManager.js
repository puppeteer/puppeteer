...
class NetworkManager extends EventEmitter {
   ...
   * @param {?{username: string, password: string}} credentials
   */
  async authenticateProxy(credentials) {
    this._proxyCredentials = credentials;
    await this._updateProtocolRequestInterception();
  }
  ...
  _onRequestIntercepted(event) {
    if (event.authChallenge) {
      ...
      const credentialsToUse = isProxy ? this._proxyCredentials : this._credentials;

      let response = 'Default';
      if (this._attemptedAuthentications.has(event.interceptionId)) {
        response = 'CancelAuth';
      } else if (credentialsToUse) {
        response = 'ProvideCredentials';
        this._attemptedAuthentications.add(event.interceptionId);
      }

      const {username, password} = credentialsToUse || {username: undefined, password: undefined};
