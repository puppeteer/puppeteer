...
import { Credentials, NetworkManagerEmittedEvents } from './NetworkManager.ts';
...
export class Page extends EventEmitter {
  ...
  async waitForFileChooser(
    ...
    let callback: (value: FileChooser) => void = null!;
  ...
  async cookies(...urls: string[]): Promise<Protocol.Network.Cookie[]> {
    ...
    const filterUnsupportedAttributes = (
      ...
      for (const attr of unsupportedCookieAttributes)
        delete cookie[attr as keyof typeof cookie];
      return cookie;
    };
  ...
  async reload(options?: WaitForOptions): Promise<HTTPResponse | null> {
    const [response] = await Promise.all<HTTPResponse | null, void>([
  ...
  async screenshot(
    ...
    let screenshotType: string = null!;
    ...
    return this._screenshotTaskQueue.postTask(() =>
      this._screenshotTask(screenshotType as 'png' | 'jpeg', options)
    );
  }
  ...
  private async _screenshotTask(
    ...
    const buffer =
      options.encoding === 'base64' ? result.data : decode(result.data);

    if (options.path) {
      typeof buffer === 'string'
        ? await Deno.writeTextFile(options.path, buffer)
        : await Deno.writeFile(options.path, buffer);
    }
    return buffer;
  ...
  async pdf(options: PDFOptions = {}): Promise<Uint8Array> {
    ...
    let paperHeight = 11;
    if (options.format) {
      const format = paperFormats[options.format.toLowerCase() as PaperFormat];
