...
class ChromeLauncher implements ProductLauncher {
  ...
  async launch(
    ...
    let temporaryUserDataDir: string = null!;
    ...
    if (!chromeArguments.some((arg) => arg.startsWith('--user-data-dir'))) {
      temporaryUserDataDir = await Deno.makeTempDir({
        prefix: 'puppeteer_dev_chrome_profile-',
      });
...
class FirefoxLauncher implements ProductLauncher {
  ...
  async _createProfile(extraPrefs: { [x: string]: unknown }): Promise<string> {
    const profilePath = await Deno.makeTempDir({
      prefix: 'puppeteer_dev_firefox_profile-',
    });
    ...
    await Deno.writeTextFile(join(profilePath, 'user.js'), userJS.join('\n'));
    await Deno.writeTextFile(join(profilePath, 'prefs.js'), prefsJS.join('\n'));
    return profilePath;
  }
}
...
function resolveExecutablePath(
  ...
  if (!launcher._isPuppeteerCore) {
    ...
    downloadPath = Deno.env.get('PUPPETEER_DOWNLOAD_PATH')!;
    assert(downloadPath);
  }
