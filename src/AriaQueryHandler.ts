...
function parseAriaSelector(selector: string): ariaQueryOption {
  ...
  const defaultName = selector.replace(
    ...
    (_, attribute: string, value: string) => {
      ...
      queryOptions[attribute as keyof ariaQueryOption] = normalize(value);
      return '';
    }
  );
...
const waitFor = async (
  ...
  const element = await domWorld.waitForSelectorInPage(
    (_: Element, selector: string) =>
      (window as any).ariaQuerySelector(selector),
    selector,
    options,
    binding
  );
