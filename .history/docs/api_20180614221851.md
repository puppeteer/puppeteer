...
#### browser.pages()
- returns: <[Promise]<[Array]<[Page]>>> Promise which resolves to an array of all open pages. Non visible pages, such as `"background_page"` and `"service_worker"`, will not be listed here. You can find them using [target.page()](#targetpage).
