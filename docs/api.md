...
- `options` <[Object]>  Set of configurable options for coverage
  - `resetOnNavigation` <[boolean]> Whether to reset coverage on every navigation. Defaults to `true`.
  - `ignoreAnonymousScripts` <[boolean]> Whether anonymous scripts generated by the page should be ignored. Defaults to `true`.
- returns: <[Promise]> Promise that resolves when coverage is started

> **NOTE** Anonymous scripts are ones that don't have an associated url. These are scripts that are either dynamically created on the page via `eval` or `new Function`, or ones injected by puppeteer using `page.evaluate`. `ignoreAnonymousScripts: false` means that the page-generated ones (`eval` and `new Function`) will be included in the coverage. Puppeteer-injected ones will always be ignored.
